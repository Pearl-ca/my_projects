...
########################################################################
zimbabwe<-read.csv("C:/Users/Kasutaja/Downloads/ipumsi_00056.csv")
zimbabwe
...
ZW
## save datafile
write.csv(ZW, file = 'Output/zimbabwe_child_2012_ipumsi.csv')

### find average child marriage  
mean(ZW$percent_married,na.rm = T)##  0.9123371%
mean(ZW$percent_males_married,na.rm = T)##  0.1457966%
mean(ZW$percent_females_married,na.rm=T)## 1.681425%

## create plot
plot(density(ZW$av_percent_males_married),
     xlim = c(0,3),
     col="blue",
     main = "Child Marriage in Zimbabwe 2012",
     xlab = "percentage",
     ylab = "Density of children married")
lines(density(ZW$av_percent_married), col = "red")                     
lines(density(ZW$av_percent_females_married), col = "green")

abline(v = mean(ZW$av_percent_married), col = "red")
abline(v = mean(ZW$av_percent_males_married), col = "blue")
abline(v = mean(ZW$av_percent_females_married), col = "green")
...
shpZW = shp[shp$CNTRY_NAME == "Zimbabwe", ]
shpZW
unique(shpZW$GEOLEVEL2)
unique(ZW$GEOLEV2)
class(shpZW$GEOLEVEL2)
class(ZW$GEOLEV2)
...
shp_ZW = merge(shpZW, ZW, by.x = "GEOLEVEL2", by.y = "GEOLEV2")
shp_ZW
...
for (j in 1:3){
  if (j == 1){
    shp_ZW$v = shp_ZW$percent_females_married
    savefile = 'percent_females_married_Zimbabwe.pdf'
    legendname = 'female'
    
  } else if (j == 2){
    shp_ZW$v = shp_ZW$percent_males_married
    savefile = 'percent_males_married_Zimbabwe.pdf'
    legendname = 'male'
    
  } else if (j == 3){
    shp_ZW$v = shp_ZW$percent_married
    savefile = 'percent_married_Zimbabwe.pdf'
    legendname = 'Total'
  }
  ...
  minnum = min(shp_ZW$v,na.rm = T)
  maxnum = max(shp_ZW$v,na.rm = T)
  minnum
  maxnum
  ...
  shp_ZW$v.qt = cut(shp_ZW$v,
                    breaks = brks,
                    include.lowest = T,
                    labels = labels,na.rm=T)
  
  brks_scale = levels(shp_ZW$v.qt)
