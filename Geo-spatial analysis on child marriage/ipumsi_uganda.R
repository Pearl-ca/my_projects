...
########################################################################
uganda<-read.csv("C:/Users/Kasutaja/Downloads/ipumsi_00054.csv")
uganda
...
UG
## save datafile
write.csv(UG, file = 'Output/uganda_child_2002_ipumsi.csv')

### find average child marriage  
mean(UG$percent_married,na.rm = T)##  1.682552%
mean(UG$percent_males_married,na.rm = T)##  0.9352646%
mean(UG$percent_females_married,na.rm=T)## 2.435958%

## create plot
plot(density(UG$av_percent_males_married),
     xlim = c(0,4),
     col="blue",
     main = "Child Marriage in Uganda 2002",
     xlab = "percentage",
     ylab = "Density of children married")
lines(density(UG$av_percent_married), col = "red")                     
lines(density(UG$av_percent_females_married), col = "green")

abline(v = mean(UG$av_percent_married), col = "red")
abline(v = mean(UG$av_percent_males_married), col = "blue")
abline(v = mean(UG$av_percent_females_married), col = "green")
...
shpUG = shp[shp$CNTRY_NAME == "Uganda", ]
shpUG
unique(shpUG$GEOLEVEL2)
unique(UG$GEOLEV2)
class(shpUG$GEOLEVEL2)
class(UG$GEOLEV2)
...
shp_UG = merge(shpUG, UG, by.x = "GEOLEVEL2", by.y = "GEOLEV2")
shp_UG
...
for (j in 1:3){
  if (j == 1){
    shp_UG$v = shp_UG$percent_females_married
    savefile = 'percent_females_married_Uganda.pdf'
    legendname = 'female'
    
  } else if (j == 2){
    shp_UG$v = shp_UG$percent_males_married
    savefile = 'percent_males_married_Uganda.pdf'
    legendname = 'male'
    
  } else if (j == 3){
    shp_UG$v = shp_UG$percent_married
    savefile = 'percent_married_Uganda.pdf'
    legendname = 'Total'
  }
  ...
  minnum = min(shp_UG$v,na.rm = T)
  maxnum = max(shp_UG$v,na.rm = T)
  minnum
  maxnum
  ...
  shp_UG$v.qt = cut(shp_UG$v,
                    breaks = brks,
                    include.lowest = T,
                    labels = labels,na.rm=T)
  
  brks_scale = levels(shp_UG$v.qt)
