...
########################################################################
ethiopia<-read.csv("C:/Users/Kasutaja/Downloads/ipumsi_00040.csv")
ethiopia
...
ET
## save datafile
write.csv(ET, file = 'Output/Ethiopia_child_2007_ipumsi.csv')

### find average child marriage  
mean(ET$percent_married,na.rm = T)##  0.7337384%
mean(ET$percent_males_married,na.rm = T)##  0.5911473%
mean(ET$percent_females_married,na.rm=T)## 0.882448%

## create plot
plot(density(ET$av_percent_males_married),
     xlim = c(0,2),
     col="blue",
     main = "Child Marriage in Ethiopia 2007",
     xlab = "percentage",
     ylab = "Density of children married")
lines(density(ET$av_percent_married), col = "red")                     
lines(density(ET$av_percent_females_married), col = "green")

abline(v = mean(ET$av_percent_married), col = "red")
abline(v = mean(ET$av_percent_males_married), col = "blue")
abline(v = mean(ET$av_percent_females_married), col = "green")
...
shpET = shp[shp$CNTRY_NAME == "Ethiopia", ]
shpET
unique(shpET$GEOLEVEL2)
unique(ET$GEOLEV2)
class(shpET$GEOLEVEL2)
class(ET$GEOLEV2)
...
shp_ET = merge(shpET, ET, by.x = "GEOLEVEL2", by.y = "GEOLEV2")
shp_ET
...
for (j in 1:3){
  if (j == 1){
    shp_ET$v = shp_ET$percent_females_married
    savefile = 'percent_females_married_Ethiopia.pdf'
    legendname = 'female'
    
  } else if (j == 2){
    shp_ET$v = shp_ET$percent_males_married
    savefile = 'percent_males_married_Ethiopia.pdf'
    legendname = 'male'
    
  } else if (j == 3){
    shp_ET$v = shp_ET$percent_married
    savefile = 'percent_married_Ethiopia.pdf'
    legendname = 'Total'
  }
  ...
  minnum = min(shp_ET$v,na.rm = T)
  maxnum = max(shp_ET$v,na.rm = T)
  minnum
  maxnum
  ...
  shp_ET$v.qt = cut(shp_ET$v,
                    breaks = brks,
                    include.lowest = T,
                    labels = labels,na.rm=T)
  
  brks_scale = levels(shp_ET$v.qt)
